...
function defaultMeetingSyncOptions() {
    return {
...
function compactWarnings(input) {
    const seen = new Set();
    const output = [];
...
async function callToolWithRetry(client, params, options = {}) {
...
function gatherPayloadCandidates(result) {
    const candidates = [];
...
function pickFirstValue(obj, patterns) {
...
function deepPickFirstValue(value, patterns, depth = 0) {
...
function parseMeetingObject(obj) {
    const idValue = pickFirstValue(obj, [/^meetingid$/, /^meetinguuid$/, /^uuid$/, /^id$/]) ??
        pickFirstValue(obj, [/meeting.*id/, /meeting.*identifier/, /meeting.*uuid/]);
    const id = stringifySafe(idValue);
    if (!id) {
        return null;
    }
    const title = stringifySafe(deepPickFirstValue(obj, [/^title$/, /^name$/, /^meetingtitle$/, /^meetingname$/, /^subject$/, /title/, /name/])) ?? 'Untitled Meeting';
    const date = stringifySafe(deepPickFirstValue(obj, [
        /^date$/,
        /^starttime$/,
        /^meetingdate$/,
        /^createdat$/,
    ...
    const attendees = toStringArray(deepPickFirstValue(obj, [/^attendees$/, /^participants$/, /^people$/, /attendees/, /participants/]));
    const notes = stringifySafe(deepPickFirstValue(obj, [
        /^notes$/,
        /^meetingnotes$/,
        /^writtennotes$/,
        /^meetingwrittennotes$/,
    ...
    const enhancedNotes = stringifySafe(deepPickFirstValue(obj, [/^enhancednotes$/, /^enhancedmeetingnotes$/, /^enhancedsummary$/, /enhancednotes/, /enhanced/])) ?? null;
    const privateNotes = stringifySafe(deepPickFirstValue(obj, [/^privatenotes$/, /^internalnotes$/, /private/, /internal.*notes/])) ?? null;
    const transcript = stringifySafe(deepPickFirstValue(obj, [/^transcript$/, /^meetingtranscript$/, /^rawtranscript$/, /transcript/])) ??
        null;
    return {
        id,
        title,
...
function collectMeetingCandidates(value, output, depth = 0) {
    ...
    const looksLikeMeeting = normalizedKeys.some((key) => key === 'meetingid' || key === 'id') &&
...
function extractMeetings(result) {
...
function inferFieldByPattern(properties, pattern) {
...
function prepareListMeetingsRequest(tool, context, options) {
    const props = schemaProperties(tool);
    const required = schemaRequired(tool);
    const args = {};
    const cursorField = inferFieldByPattern(props, /^cursor$/) ?? inferFieldByPattern(props, /nextcursor|cursor/);
    const pageField = inferFieldByPattern(props, /^page$/) ?? inferFieldByPattern(props, /pagenumber/);
    const pageSizeField = inferFieldByPattern(props, /^pagesize$/) ?? inferFieldByPattern(props, /^limit$/) ?? inferFieldByPattern(props, /page.*size|limit/);
    const timeRangeField = inferFieldByPattern(props, /^timerange$/) ??
...
function extractPaginationCursor(result) {
    const cursor = deepPickFirstValue(result, [/^nextcursor$/, /^cursor$/, /next.*cursor/, /cursor/]);
    return typeof cursor === 'string' && cursor.trim() ? cursor.trim() : null;
}
function extractHasMore(result) {
    const value = deepPickFirstValue(result, [/^hasmore$/, /^hasnext$/, /has.*more/, /has.*next/]);
    return value === true;
}
function buildGetMeetingsArguments(tool, ids) {
    const props = schemaProperties(tool);
    const required = schemaRequired(tool);
    const args = {};
    const idsField = inferFieldByPattern(props, /^meetingids$/) ??
...
function buildGetMeetingTranscriptArguments(tool, id) {
    const props = schemaProperties(tool);
    const required = schemaRequired(tool);
    const args = {};
    const idField = inferFieldByPattern(props, /^meetingid$/) ??
...
function buildQueryArguments(tool, message, meetingIds) {
    const props = schemaProperties(tool);
    const required = schemaRequired(tool);
    const args = {};
    const messageField = inferFieldByPattern(props, /^query$/) ??
    ...
    if (meetingIds?.length) {
        const idsField = inferFieldByPattern(props, /^documentids$/) ??
...
function extractTranscriptFromResult(result) {
...
export async function loadMeetingsWithClient(client, inputOptions = {}) {
    const options = {
    ...
    const tools = Array.isArray(toolsResponse.tools)
        ? toolsResponse.tools.filter((tool) => isRecord(tool) && typeof tool.name === 'string')
        : [];
    ...
    for (let page = 0; page < maxPages; page += 1) {
        ...
        const result = await callToolWithRetry(client, {
    ...
    if (getMeetingsTool && meetingsById.size > 0 && options.maxGetMeetingsPerSync > 0) {
        ...
        if (ids.length > 0) {
            try {
                ...
                const details = await callToolWithRetry(client, {
        ...
        if (globalCooldownUntilMs != null && globalCooldownUntilMs > now) {
            ...
            for (const meeting of transcriptTargets) {
                try {
                    ...
                    const transcriptResult = await callToolWithRetry(client, {
...
export async function queryGranolaMeetings(client, message, meetingIds) {
    const toolsResponse = (await client.listTools());
    const tools = Array.isArray(toolsResponse.tools)
        ? toolsResponse.tools.filter((tool) => isRecord(tool) && typeof tool.name === 'string')
        : [];
