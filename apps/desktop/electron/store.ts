...
function assertNotePatch(patch: unknown): asserts patch is NoteUpdatePatch {
  if (!isRecord(patch)) {
...
function assertSettingsPatch(patch: unknown): asserts patch is AppSettingsPatch {
  if (!isRecord(patch)) {
...
export class InMemoryGranolaService {
  ...
  updateNote(id: unknown, patch: unknown): Note {
    assertId(id);
